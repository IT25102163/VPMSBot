...
public class SecurityServlet extends HttpServlet {
    ...
    private synchronized void handleEntry(HttpServletRequest req,
        ...
        try {
            ...
            int slotNum = Integer.parseInt(freeSlot.slotId.replace("P", ""));
            new ParkingLotDAO().recordEntry(vehicleNum.toUpperCase(), uid, slotNum);
        } catch (Exception e) {
            System.out.println("[Security] DB save failed: " + e.getMessage());
        }
    ...
    private synchronized void handleExit(HttpServletRequest req,
        ...
        // ── FREE RESERVED SLOT IN DB ──────────────────────────────────
        // If this vehicle had a reservation, set that slot back to Available
        try {
            for (Reservation r : ReservationServlet.reservations) {
                if (r.getVehicleNo().equalsIgnoreCase(vehicleNum)) {
                    ...
                    System.out.println("[Security] Freed reserved slot " + r.getSlotId()
                            + " for vehicle " + vehicleNum);
                    break;
                }
            }
        } catch (Exception e) {
        ...
        System.out.printf("[Security] EXIT vehicle=%s slot=%s%n",
                vehicleNum, entry.slotId);
