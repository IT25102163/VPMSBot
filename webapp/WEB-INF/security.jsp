...
<%
  ...
  boolean isAdmin = "admin".equals(secRole);

  // For regular users — auto fill their vehicle
  int secUserId = 0;
  String secVehicle = "";
  try {
...
.admin-hint{background:#eef2ff;border:1px solid #c7d2fe;border-radius:8px;padding:10px 13px;font-size:12px;color:#4338ca;margin-bottom:12px;}
...
<div class="main">
  ...
  <div class="content">
    ...
    <div class="top-grid">
      ...
      <div class="card">
        <div class="card-hdr">
          <div class="card-title"><div class="card-ico" style="background:#DCFCE7;">🟢</div>Vehicle Entry</div>
        </div>
        <div class="card-body">
          <div id="entryAlert"></div>
          <% if (isAdmin) { %>
          <div class="admin-hint">ℹ️ As admin you can enter any vehicle number manually.</div>
          <% } %>
          <div class="form-group">
            <label class="form-label">Vehicle Number</label>
            <!-- Admin: editable. User: locked to their vehicle -->
            <input class="form-input <%= isAdmin ? "" : "locked" %>"
                   id="entryVehicle" type="text"
                   value="<%= secVehicle %>"
                   placeholder="e.g. CAR-1234"
                   <%= isAdmin ? "oninput=\"this.value=this.value.toUpperCase()\"" : "readonly" %>/>
          </div>
          <div class="form-group">
            <label class="form-label">User ID</label>
            <input class="form-input <%= isAdmin ? "" : "locked" %>"
                   id="entryUserId" type="<%= isAdmin ? "number" : "text" %>"
                   value="<%= secUserId %>"
                   placeholder="e.g. 1"
                   <%= isAdmin ? "" : "readonly" %>/>
          </div>
          <div class="form-group">
            <label class="form-label">Slot ID (which slot to occupy)</label>
            <input class="form-input" id="entrySlotId" type="number"
                   placeholder="e.g. 1 for A01, 2 for A02" min="1"/>
          </div>
          <div class="form-group">
      ...
      <div class="card">
        <div class="card-hdr">
          <div class="card-title"><div class="card-ico" style="background:#FEE2E2;">🔴</div>Vehicle Exit</div>
        </div>
        <div class="card-body">
...
const MY_VEHICLE = '<%= secVehicle %>';
const MY_USERID  = '<%= secUserId %>';
...
async function recordEntry() {
  ...
  const userId  = document.getElementById('entryUserId').value.trim();
  const slotId  = document.getElementById('entrySlotId').value.trim();
  const type    = document.getElementById('entryType').value;

  if (!vehicle) { showAlert('entryAlert','Enter vehicle number!','warning'); return; }
  if (!userId)  { showAlert('entryAlert','Enter user ID!','warning'); return; }
  if (!slotId)  { showAlert('entryAlert','Enter slot ID to occupy!','warning'); return; }
  ...
  try {
    ...
    if (data.success) {
      showAlert('entryAlert','✅ Entry recorded! Slot '+slotId+' is now Occupied (red).','success');
      if (IS_ADMIN) {
        document.getElementById('entryVehicle').value = '';
        document.getElementById('entryUserId').value  = '';
        document.getElementById('entrySlotId').value  = '';
      }
      loadLog(); loadStack(); loadStats();
    } else { showAlert('entryAlert','❌ '+data.message,'error'); }
...
async function recordExit() {
  ...
  if (!vehicle) { showAlert('exitAlert','Enter vehicle number!','warning'); return; }
  try {
    ...
    const data = await res.json();
    if (data.success) {
      showAlert('exitAlert','✅ Exit recorded! Slot is now free (green).','success');
      loadLog(); loadStack(); loadStats();
    } else { showAlert('exitAlert','❌ '+data.message,'error'); }
...
async function loadLog() {
  try {
    ...
    if (!data.length) { body.innerHTML='<tr><td colspan="6" style="text-align:center;color:#64748b;padding:24px;">No records yet.</td></tr>'; return; }
    body.innerHTML = data.map(e=>
      '<tr><td><span class="vtag">'+e.vehicle+'</span></td>'+
      ...
      '<td><span class="'+(e.status==='IN'?'in-badge':'out-badge')+'">'+e.status+'</span></td></tr>'
    ).join('');
  } catch(e) {}
}
...
async function loadStack() {
  try {
    ...
    if (!data.length) { body.innerHTML='<tr><td colspan="3" style="text-align:center;color:#64748b;padding:24px;">No vehicles parked.</td></tr>'; return; }
    body.innerHTML = data.map(e=>
      '<tr><td><span class="vtag">'+e.vehicle+'</span></td>'+
      '<td><code style="font-size:11px;background:#f1f5f9;padding:3px 7px;border-radius:5px;">'+e.slot+'</code></td>'+
      '<td style="font-size:12px;">'+e.entry+'</td></tr>'
    ).join('');
  } catch(e) {}
}
